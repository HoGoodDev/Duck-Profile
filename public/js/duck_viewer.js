...
export class DuckViewer {
  ...
  setDuckColors(duck) {
    ...
    const duck_colors = {
      ...
      front_left: duck.body?.frontLeft ?? duck.body?.front1 ?? "yellow", //frpnt1 fallback to check for old naming system
    ...
    this._duckObject.traverse((child) => {
      ...
      const setColor = (m, key) => {
        if (!m || !m.color) return;
        const chosen = duck_colors[key] ?? duck_colors[m.name] ?? "yellow";
        m.color.set(chosen);
      };
